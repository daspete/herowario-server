    ...
    Subscription: {
        UserLoggedIn: {
            subscribe(root, args, context){
                return DataPublisher.asyncIterator('user.login')
            }
        }
    },

    Mutation: {
        async Login(parent, { email, password }, { UserProvider }){
            let userQuery = {}
            ...
            if(!user) throw new UserInputError('not_found')

            const passwordCheck = await bcrypt.compare(password, user.password)
            if(!passwordCheck) throw new UserInputError('not_found')
